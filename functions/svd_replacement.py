...
class H_functions:
    ...
    def H(self, vec):
        ...
        """
        #vec: (1,32,256,256)
        temp = self.Vt(vec) #（1，2097152） 
        singulars = self.singulars() # (629910)
    ...
    def H_pinv(self, vec): 
        ...
        """
        temp = self.Ut(vec) #vec & temp: [1, 629910]
        singulars = self.singulars()#[629910]
        temp[:, :singulars.shape[0]] = temp[:, :singulars.shape[0]] / singulars #[1, 629910]
...
class Inpainting(H_functions):
    def __init__(self, channels, img_dim, missing_indices, keeping_indices, device):
        self.channels = channels
        self.img_dim = img_dim
        self._singulars = torch.ones(channels * img_dim**2 - missing_indices.shape[0]).to(device) #[629910]
        self.missing_indices = missing_indices.to(device).long()
        self.kept_indices = keeping_indices.to(device).long()
    ...
    def V(self, vec):
        ...
        return out.reshape(vec.shape[0], -1, self.channels).permute(0, 2, 1).reshape(vec.shape[0], -1) # [1, 2097152] ---> [1, 256*256, 32] ---> [1, 32, 256*256] ---> [1, 2097152]

    def Vt(self, vec): #vec:(1,32,256,256)
        #该函数将图像展成向量之后，重排列
        temp = vec.clone().reshape(vec.shape[0], self.channels, -1).permute(0, 2, 1).reshape(vec.shape[0], -1) #(1,32,256,256) ---> (1,32,65536) ---> (1,65536,32) ---> （1， 2097152） 
        out = torch.zeros_like(temp) #（1， 2097152）
        # 下面两行代码将keep的元素放在前面，mask掉的东西放在后面
...
class SuperResolution(H_functions):
    ...
    def Vt(self, vec):
        ...
        patches = vec.clone().reshape(vec.shape[0], self.channels, self.img_dim, self.img_dim) # (1,3,256, 256）
        patches = patches.unfold(2, self.ratio, self.ratio) # (1, 3, 64, 256, 4)
        patches = patches.unfold(3, self.ratio, self.ratio)# (1, 3, 64, 64, 4, 4)
        unfold_shape = patches.shape
        patches = patches.contiguous().reshape(vec.shape[0], self.channels, -1, self.ratio**2) # (1, 3, 4096, 16)
        #multiply each by the small V transposed
        patches = torch.matmul(self.Vt_small, patches.reshape(-1, self.ratio**2, 1)).reshape(vec.shape[0], self.channels, -1, self.ratio**2) # (1, 3, 4096, 16)
        #reorder the vector to have the first entry first (because singulars are ordered descendingly)
        recon = torch.zeros(vec.shape[0], self.channels * self.img_dim**2, device=vec.device) # (1, 3 * 256 * 256)
        #        :3*64*64
